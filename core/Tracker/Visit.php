...
class Visit implements VisitInterface
{
    ...
    {
        $this->addParams($visit);

        return $this->getModel()->createVisit($visit);
    }

    function addParams(&$visitInfo){

        $Params = $this->request->getParams();
        $user_id = addslashes(!empty($Params['user_id'])?$Params['user_id']:'');
        if($user_id){
            $visitInfo['user_id'] = $user_id;
        }
        if($Params['cip']){
            $visitInfo['location_ip'] = IPUtils::stringToBinaryIP($Params['cip']);
        }
    }
}
