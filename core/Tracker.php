...
define('Tracker_Token_Valid', "joymepiwik");
//define('Redis_Piwik_Table', "piwik_visit_key2");
define('Redis_Piwik_Table', "piwik_visit_new");

//alpha
define('Redis_Piwik_Host', "172.16.75.32");
define('Redis_Piwik_Port', 6379);
//prod
//define('Redis_Piwik_Host', "192.168.30.102");
//define('Redis_Piwik_Port', 6380);
...
class Tracker {
    ...
    public function initRequests($args) {
    ...
    private function authenticateBulkTrackingRequests($rawData) {
        ...
        if (!empty($this->requests)) {
            foreach ($this->requests as &$request) {
                ...
                if ($bulkTrackingRequireTokenAuth && !$requestObj->isAuthenticated()
                ) {
     ...
     */
    public function inputByCache($params = null) {
        if (empty($params)) {
            return $this->handleEmptyRequest();
        }
        $tokenAuth = Tracker_Token_Valid;
    ...
    protected function isVisitValid() {
        return $this->stateValid !== self::STATE_LOGGING_DISABLE && $this->stateValid !== self::STATE_EMPTY_REQUEST;
    }
    ...
    protected function runScheduledTasksIfAllowed($isAuthenticated) {
        ...
        try {
            if (!$isAuthenticated && $this->shouldRunScheduledTasks()
            ) {
